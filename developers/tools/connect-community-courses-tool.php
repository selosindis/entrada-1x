...
if (((int)$community["community_protected"])) {
	$public_view = 0;
} else {
	$public_view = 1;
}
if (!((int)$community["community_registration"])) {
	$troll_view = 1;
} else {
	$troll_view = 1;
}
$time = time();
if (!$db->Execute("INSERT INTO `community_pages` (`community_id`, `parent_id`, `page_order`, `page_type`, `menu_title`, `page_title`, `page_url`, `page_content`, `page_active`, `page_visible`, `allow_member_view`, `allow_troll_view`, `allow_public_view`, `updated_date`, `updated_by`) VALUES
					(".$db->qstr($COMMUNITY_ID).", 0, 0, 'course', 'Background', 'Background Information', '', ' ', 1, 1, 1, ".$db->qstr($troll_view).", ".$db->qstr($public_view).", ".$db->qstr($time).", 1),
					(".$db->qstr($COMMUNITY_ID).", 0, 1, 'course', 'Course Calendar', 'Course Calendar', 'course_calendar', ' ', 1, 1, 1, ".$db->qstr($troll_view).", ".$db->qstr($public_view).", ".$db->qstr($time).", 1),
					(".$db->qstr($COMMUNITY_ID).", 0, 2, 'course', 'Prerequisites', 'Prerequisites (Foundational Knowledge)', 'prerequisites', ' ', 1, 1, 1, ".$db->qstr($troll_view).", ".$db->qstr($public_view).", ".$db->qstr($time).", 1),
					(".$db->qstr($COMMUNITY_ID).", 0, 3, 'course', 'Course Aims', 'Aims of the Course', 'course_aims', ' ', 1, 1, 1, ".$db->qstr($troll_view).", ".$db->qstr($public_view).", ".$db->qstr($time).", 1),
					(".$db->qstr($COMMUNITY_ID).", 0, 4, 'course', 'Learning Objectives', 'Learning Objectives', 'objectives', ' ', 1, 1, 1, ".$db->qstr($troll_view).", ".$db->qstr($public_view).", ".$db->qstr($time).", 1),
					(".$db->qstr($COMMUNITY_ID).", 0, 5, 'course', 'MCC Presentations', 'MCC Presentations', 'mcc_presentations', ' ', 1, 1, 1, ".$db->qstr($troll_view).", ".$db->qstr($public_view).", ".$db->qstr($time).", 1),
					(".$db->qstr($COMMUNITY_ID).", 0, 6, 'course', 'Teaching Strategies', 'Teaching and Learning Strategies', 'teaching_strategies', ' ', 1, 1, 1, ".$db->qstr($troll_view).", ".$db->qstr($public_view).", ".$db->qstr($time).", 1),
					(".$db->qstr($COMMUNITY_ID).", 0, 7, 'course', 'Assessment Strategies', 'Assessment Strategies', 'assessment_strategies', ' ', 1, 1, 1, ".$db->qstr($troll_view).", ".$db->qstr($public_view).", ".$db->qstr($time).", 1),
					(".$db->qstr($COMMUNITY_ID).", 0, 8, 'course', 'Resources', 'Resources', 'resources', ' ', 1, 1, 1, ".$db->qstr($troll_view).", ".$db->qstr($public_view).", ".$db->qstr($time).", 1),
					(".$db->qstr($COMMUNITY_ID).", 0, 9, 'course', 'Expectations of Students', 'What is Expected of Students', 'expectations_of_students', ' ', 1, 1, 1, ".$db->qstr($troll_view).", ".$db->qstr($public_view).", ".$db->qstr($time).", 1),
					(".$db->qstr($COMMUNITY_ID).", 0, 10, 'course', 'Expectations of Faculty', 'What is Expected of Course Faculty', 'expectations_of_faculty', ' ', 1, 1, 1, ".$db->qstr($troll_view).", ".$db->qstr($public_view).", ".$db->qstr($time).", 1)")) {
