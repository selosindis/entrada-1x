...
dhx.i18n = {
    ...
    setLocale:function(){
    	var helpers = ["fullDateFormat", "timeFormat", "dateFormat", "longDateFormat"];
...
dhx.protoUI({
	...
	resizeChildren:function(structure_changed){ 
		if (structure_changed && this.type !="clean"){
			...
			for (var i = 0; i < this._cells.length; i++){
				...
				cell._viewobj.style.borderTopWidth=cell._viewobj.style.borderBottomWidth=cell._viewobj.style.borderLeftWidth=cell._viewobj.style.borderRightWidth="1px";
			}
	...
	_afterResetBorders:function(collection){
		...
		if (this._padding){
			...
			this._cells[0]._viewobj.style[contrstyle] = this._padding+"px";
		}
				
		var style = this._vertical_orientation?"marginTop":"marginLeft";
		var index = 0;
		for (index; index<start; index++)
			this._cells[index]._viewobj.style[style]="0px";
		for (index; index<collection.length; index++)
			this._cells[index]._viewobj.style[style]=this._margin+"px";
...
dhx.protoUI({
    ...
    _getDateBoundaries: function() { // addition information about rendering event: how many days from the previous month, next, number of weeks to display and so on
	    ...
	    this._heightOffset = this._settings.monthHeaderHeight + (s.weekHeader?this._settings.weekHeaderHeight:0) + (s.timeSelect?this._settings.timeSelectHeight:0);
    },
	...
	_getColumnSizes: function(){
        ...
        if(s.cellHeight<=0) {
            ...
            for (var p = 0; p < this._rowcount; p++) {
                this._columnsHeight[p] = s.cellHeight;
            }
        }
	},
	...
	render: function() {
		...
		if(s.weekHeader)
            html += "<div class='dhx_cal_header' style='height:"+this._settings.weekHeaderHeight+"px'>"+this._week_template()+"</div>";
...
dhx.protoUI({
	...
	_types:{
		...
		big:["<div class='dhx_el_bigbutton'><input type='button' value='#label#'></div>",28],
		biground:["<div class='dhx_el_bigroundbutton'><input type='button' value='#label#'></div>",28]
	},
	$setSize:function(x,y){
		if(dhx.ui.view.prototype.$setSize.call(this,x,y)){
			this.render();
		...
		if(dhx.AtomRender.render.call(this)){
			...
			if (this._settings.align){
				switch(this._settings.align){
					case "right":
						//IE8COMPAT
						this._dataobj.firstChild.style.cssFloat = this._dataobj.firstChild.style.styleFloat = "right";
						break;
					case "center":
...
dhx.protoUI({
	...
	getList: function(){
		return dhx.ui.get(this._settings.id+"_list");
	},
	_setValue:function(value){
...
dhx.protoUI({
	...
	_setValue:function(value){
		...
		if(typeof value=="string"&&value){
			value = (this._settings.dateFormat||dhx.i18n.dateFormatDate)(value);
		}
...
dhx.protoUI({
	...
	_set_inner_size:function(){
      	var  width = (this._settings.inputWidth||(this._content_width-this._settings._cssConstant))-this._settings.labelWidth;
      	var box = this._dataobj.childNodes[1];
      		box.style.width = width+"px";
      	this._bar_width = width-this._settings._cssConstant;
		box.childNodes[1].style.width = this._bar_width+"px";

		if(!this._settings.options)
			this.define("options",[{value:"0", label: ""}]);
    ...
    _reflectValue:function(left){
    	...
    	box.lastChild.style.left = left-10+"px";
    ...
    _onTouchMove:function(start, current){
    	if (this._slider_drag){
    		this._reflectValue(this._slider_left - (start.x - current.x));
    	}
    },
