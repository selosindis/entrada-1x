 ...
 * 
 * @version $Id: agent-accommodation.php 1171 2010-05-01 14:39:27Z ad29 $
*/
...
if((!isset($_SESSION["isAuthorized"])) || (!$_SESSION["isAuthorized"])) {
	...
	switch($STEP) {
		case "2" :
			...
			$mail->Body		= $message;
			$mail->ClearAddresses();
			$mail->AddAddress($AGENT_CONTACTS["agent-accommodation"]["email"], $AGENT_CONTACTS["agent-accommodation"]["name"]);
			if($mail->Send()) {
				$SUCCESS++;
			...
			<div id="wizard-footer" style="position: absolute; top: 465px; left: 0px; width: 100%; height: 40px; border-top: 2px #CCCCCC solid; padding: 4px 4px 4px 10px">
				...
				<tr>
					...
					<td style="width: 272px; text-align: right">
						<input type="button" class="button" value="New Issue" onclick="window.location='<?php echo ENTRADA_URL; ?>/agent-accommodation.php'" />
					</td>
				</tr>
				</table>
			</div>
		...
		default :
			?>
			<form id="issue-form" action="<?php echo ENTRADA_URL; ?>/agent-accommodation.php?step=2" method="post" style="display: inline">
