...
if ((!isset($_SESSION["isAuthorized"])) || (!$_SESSION["isAuthorized"])) {
	...
	if ($parent_id) {
		$query = "	SELECT a.*, b.`organisation_id`
					...
					WHERE a.`event_id` = ".$db->qstr($parent_id)."
					AND b.`course_active` = '1'";
		...
		if (!$ERROR) {
			...
			if (($event_info && $db->AutoExecute("events", $PROCESSED, "UPDATE", "`event_id` = ".$db->qstr($event_id))) || ((!$event_info && $db->AutoExecute("events", $PROCESSED, "INSERT")))) {
				...
				if ($db->Execute($query)) {
					...
					if ((!isset($PROCESSED["associated_proxy_ids"]) || !count($PROCESSED["associated_proxy_ids"])) && (!isset($PROCESSED["associated_group_ids"]) || !count($PROCESSED["associated_group_ids"]))) {
	...
	if ($display_controls) {
		...
		<table style="width: 100%" cellspacing="0" cellpadding="0" border="0" summary="Event Session Information">
			...
			<tr>
				...
				<td>
					<textarea id="session_description" name="session_description" style="width: 90%; height: 80px" cols="70" rows="10"><?php echo html_encode(trim(strip_selected_tags($PROCESSED["session_description"], array("font")))); ?></textarea>
				</td>
			</tr>
			<tr>
				<td colspan="3">&nbsp;</td>
			...
			<tr>
				...
				<td>
					<textarea id="session_message" name="session_message" style="width: 90%; height: 80px" cols="70" rows="10"><?php echo html_encode(trim(strip_selected_tags($PROCESSED["session_message"], array("font")))); ?></textarea>
				</td>
			</tr>
			<tr>
				<td colspan="3">&nbsp;</td>
