...
if ($RECORD_ID) {
	...
	if ($discussion_record) {
		if ((int) $discussion_record["forum_active"]) {
				...
				case 2 :
					...
					if (!$ERROR) {
						...
						if ($db->AutoExecute("community_discussions", $PROCESSED, "UPDATE", "`community_id` = ".$db->qstr($COMMUNITY_ID)." AND `cdiscussion_id` = ".$db->qstr($RECORD_ID))) {
							...
							$SUCCESS++;
							$SUCCESSSTR[]	= "You have successfully updated the ".html_encode($PROCESSED["forum_title"])."</strong> discussion forum.<br /><br />You will now be redirected to the index; this will happen automatically</strong> in 5 seconds or <a href=\"".$url."\" style=\"font-weight: bold\">click here</a> to continue.";
			...
			$NOTICE++;
			$NOTICESTR[] = "The discussion forum that you are trying to edit was deactivated ".date(DEFAULT_DATE_FORMAT, $discussion_record["updated_date"])."</strong> by ".html_encode(get_account_data("firstlast", $discussion_record["updated_by"]))."</strong>.<br /><br />If there has been a mistake or you have questions relating to this issue please contact the MEdTech Unit directly.";
