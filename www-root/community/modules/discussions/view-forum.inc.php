		...
		if (discussions_module_access($RECORD_ID, "view-forum")) {
			...
			<div style="padding-top: 10px; clear: both">
				...
				if ($results) {
					...
					<table class="table table-striped table-bordered">
						...
						<thead>
							<tr>
								...
								<td<?php echo (($_SESSION[APPLICATION_IDENTIFIER]["cid_".$COMMUNITY_ID][$PAGE_URL]["sb"] == "replies") ? " class=\"sorted".strtoupper($_SESSION[APPLICATION_IDENTIFIER]["cid_".$COMMUNITY_ID][$PAGE_URL]["so"])."\"" : ""); ?> style="text-align: left"><?php echo communities_order_link("replies", "Replies"); ?></td>
								<td<?php echo (($_SESSION[APPLICATION_IDENTIFIER]["cid_".$COMMUNITY_ID][$PAGE_URL]["sb"] == "poster") ? " class=\"sorted".strtoupper($_SESSION[APPLICATION_IDENTIFIER]["cid_".$COMMUNITY_ID][$PAGE_URL]["so"])."\"" : ""); ?>><?php echo communities_order_link("poster", "Topic Starter"); ?></td>
								<td<?php echo (($_SESSION[APPLICATION_IDENTIFIER]["cid_".$COMMUNITY_ID][$PAGE_URL]["sb"] == "date") ? " class=\"sorted".strtoupper($_SESSION[APPLICATION_IDENTIFIER]["cid_".$COMMUNITY_ID][$PAGE_URL]["so"])."\"" : ""); ?>><?php echo communities_order_link("date", "Latest Action"); ?></td>
							</tr>
						</thead>
						<tbody>
						<?php
						foreach($results as $key => $result) {
							...
							echo "		By:</strong> ".$latest_poster_display."\n";
							echo "	</td>\n";
							echo "</tr>\n";
					...
					$NOTICE++;
					$NOTICESTR[] = "No topics in this forum.</strong><br /><br />".((discussions_module_access($RECORD_ID, "add-post")) ? "If you would like to create a new post, <a href=\"".COMMUNITY_URL.$COMMUNITY_URL.":".$PAGE_URL."?section=add-post&id=".$RECORD_ID."\">click here</a>." : "Please check back later.");
