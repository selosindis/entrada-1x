...
if($COMMUNITY_ID) {
	...
	if($community_details) {
		...
		$BREADCRUMB[]		= array("url" => ENTRADA_URL."/admin/communities?".replace_query(), "title" => "Manage Members---");
		if($ENTRADA_ACL->amIAllowed(new CommunityResource($COMMUNITY_ID), 'update')) {
								...
								if($GUEST_PROXY_ID > 0) {
									...
									if($GUEST_ACCESS) {
										...
										if($member_add_success) {
											$url = ENTRADA_URL."/admin/communities?section=members&community=".$COMMUNITY_ID;
											$SUCCESS++;
											if($GUEST_NEW_ACCESS) {
			...
			switch($STEP) {
				case 3 :
					...
					break;
				case 2 :
					$ONLOAD[]		= "setTimeout('window.location=\\'".ENTRADA_URL."/admin/".$MODULE."?section=members&community=".$COMMUNITY_ID."\\'', 5000)";
...
<div class="tab-pane" id="community_members_div">
	<div class="tab-page members">
		...
		<h2 style="margin-top: 0px">Community Members</h2>
							...
							if($result) {
								...
								if($TOTAL_PAGES > 1) {
									$member_pagination = new Pagination($PAGE_CURRENT, $_SESSION[APPLICATION_IDENTIFIER][$MODULE]["pp"], $TOTAL_ROWS, ENTRADA_URL."/admin/".$MODULE, replace_query(), "mpv");
								} else {
							...
							if($results) {
								...
								?>
								<form action="<?php echo ENTRADA_URL."/admin/".$MODULE."?".replace_query(array("section" => "members", "type" => "members", "step" => 2)); ?>" method="post">
	...
	<div class="tab-page members">
		...
		<h2 style="margin-top: 0px">Community Administrators</h2>
							...
							if($result) {
								...
								if($TOTAL_PAGES > 1) {
									$admin_pagination = new Pagination($PAGE_CURRENT, $_SESSION[APPLICATION_IDENTIFIER][$MODULE]["pp"], $TOTAL_ROWS, ENTRADA_URL."/admin/".$MODULE, replace_query(), "apv");
								} else {
							...
							if($results) {
								...
								?>
		<form action="<?php echo ENTRADA_URL."/admin/".$MODULE."?".replace_query(array("section" => "members", "type" => "admins", "step" => 2)); ?>" method="post">
	...
	<div class="tab-page members">
		...
		<form action="<?php echo ENTRADA_URL."/admin/".$MODULE."?".replace_query(array("section" => "members", "type" => "add", "step" => 2)); ?>" method="post">
					...
					<tr>
						<td colspan="2" style="vertical-align: top">
							<div class="member-add-type" id="existing-member-add-type">
								...
								<input id="community_member_ids" name="community_member_ids" value="<?php  echo (isset($added_proxy_ids) && $added_proxy_ids ? implode(",", $added_proxy_ids) : "") ?>" style="display: none;">
							</div>
						</td>
	...
	<div class="tab-page members">
		...
		<form action="<?php echo ENTRADA_URL."/admin/".$MODULE."?".replace_query(array("section" => "members", "type" => "addguest", "step" => 2)); ?>" method="post">
...
<script type="text/javascript">
	...
	function updatePeopleList(newoptions, index) {
		if ($('community_members_index').value == index) {
			...
			var member_ids = "";
			if ($F('community_members')) {
				ids[index] = $F('community_members').split(',').compact();
				for (i=0; i < ids.length; i++) {
					if (ids[i] !== undefined && ids[i].length > 0) {
						if (!member_ids) {
							member_ids = ids[i].join(',');
						} else {
							member_ids = member_ids + ',' + ids[i].join(',');
						}
					}
				}
				$('community_member_ids').value = member_ids;
			}
			ids[index] = $F('community_members').split(',').compact();
	...
	$('community_members_category_select').observe('change', function(event) {
		if ($('community_members_category_select').selectedIndex != 0) {
			...
			var updater = new Ajax.Updater('community_members_scroll', '<?php echo ENTRADA_URL."/admin/communities?section=membersapi&action=memberlist";?>',{
				method:'post',
				parameters: {
...
<br /><br />
	...
	header("Location: ".ENTRADA_URL."/admin/communities");
	exit;
}
?>