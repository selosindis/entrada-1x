...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_CONFIGURATION"))) {
	...
	$BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/configuration/organisations/manage/metadata?".replace_query(array("section" => "add"))."&amp;org=".$ORGANISATION_ID, "title" => "Edit Meta Data");
	...
	// Error Checking
				...
				if (!$ERROR) {
					...
					if ($db->AutoExecute("meta_types", $PROCESSED, "UPDATE","`meta_type_id` = ".$db->qstr($PROCESSED["meta_type_id"]))) {
						if(isset($PROCESSED["groups"]) && is_array($PROCESSED["groups"])){
							...
							$url = ENTRADA_URL . "/admin/configuration/organisations/manage/metadata?org=".$ORGANISATION_ID;
							$SUCCESS++;
		...
		switch ($STEP) {
			...
			default:	
				...
				?>
				<form action="<?php echo ENTRADA_URL."/admin/configuration/organisations/manage/metadata?".replace_query(array("action" => "add", "step" => 2))."&org=".$ORGANISATION_ID; ?>" method="post">
