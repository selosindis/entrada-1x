...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_OBJECTIVES"))) {
	...
	switch($STEP) {
		...
		default :
			...
			if (!count($objectives)) {
				header("Location: ".ENTRADA_URL."/admin/configuration/organisations/manage/objectives");
				exit;
			}
		break;
	}
	...
	switch($STEP) {
		...
		default :
			if ($ERROR) {
				...
				?>
				<form action="<?php echo ENTRADA_URL."/admin/configuration/organisations/manage/objectives?".replace_query(array("action" => "delete", "step" => 2)); ?>" method="post">
