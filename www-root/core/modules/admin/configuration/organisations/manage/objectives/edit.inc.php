...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_OBJECTIVES"))) {
	...
	$ONLOAD[]	= "setTimeout('window.location=\\'".ENTRADA_URL."/admin/configuration/organisations/manage/".$MODULE."\\'', 15000)";
	...
	if ($OBJECTIVE_ID) {
		...
		if ($objective_details) {
			$BREADCRUMB[]	= array("url" => ENTRADA_URL."/admin/configuration/organisations/manage/objectives?".replace_query(array("section" => "edit")), "title" => "Editing Objective");
			...
			switch ($STEP) {
				...
				case 1:
					...
					?>
					<form action="<?php echo ENTRADA_URL."/admin/configuration/organisations/manage/objectives"."?".replace_query(array("action" => "add", "step" => 2)); ?>" method="post">
