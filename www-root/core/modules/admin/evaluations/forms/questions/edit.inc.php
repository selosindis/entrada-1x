...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_EVALUATIONS"))) {
	...
	if ($FORM_ID) {
		...
		if ($quiz_record &&  $ENTRADA_ACL->amIAllowed(new EvaluationFormResource($quiz_record["quiz_id"]), "update")) {
			if ($ALLOW_QUESTION_MODIFICATIONS) {
				...
				switch ($STEP) {
					case 2 :
						...
						if (!$ERROR) {
							if($ENTRADA_ACL->amIAllowed(new EvaluationFormQuestionResource($FORM_ID, $quiz_record["quiz_id"]), "update")) {
								if ($db->AutoExecute("quiz_questions", $PROCESSED, "UPDATE", "`qquestion_id` = ".$db->qstr($FORM_ID))) {
									...
									if ((is_array($PROCESSED["quiz_question_responses"])) && (count($PROCESSED["quiz_question_responses"]))) {
										foreach ($PROCESSED["quiz_question_responses"] as $quiz_question_response) {
											...
											if (!$db->AutoExecute("quiz_question_responses", $PROCESSED, "INSERT")) {
												...
												application_log("error", "Unable to insert a new quiz_question_responses record while adding a new quiz question [".$FORM_ID."] to quiz_id [".$quiz_record["quiz_id"]."]. Database said: ".$db->ErrorMsg());
											}
										}
									}
								} else {
									...
									application_log("error", "Unable to update a quiz question [".$FORM_ID."] to quiz_id [".$quiz_record["quiz_id"]."] due to a lack of permissions.");
								...
								application_log("success", "Updated quiz question [".$FORM_ID."] in quiz_id [".$quiz_record["quiz_id"]."].");
							} else {
								$ERROR++;
								...
								application_log("error", "There was an error updating quiz question [".$FORM_ID."] in quiz_id [".$quiz_record["quiz_id"]."]. Database said: ".$db->ErrorMsg());
							}
						}
					...
					default :
						...
						$query		= "	SELECT a.*
										...
										WHERE a.`qquestion_id` = ".$db->qstr($FORM_ID)."
				...
				switch ($STEP) {
					...
					default :
						...
						<form action="<?php echo ENTRADA_URL; ?>/admin/<?php echo $MODULE; ?>?section=edit-question&amp;id=<?php echo $FORM_ID; ?>&amp;step=2" method="post" id="editQuizQuestionForm">
								...
								<td colspan="3" style="padding-top: 25px">
									...
									<tr>
										<td style="width: 25%; text-align: left">
											<input type="button" class="button" value="Cancel" onclick="window.location='<?php echo ENTRADA_URL; ?>/admin/<?php echo $MODULE; ?>?section=edit&amp;id=<?php echo $quiz_record["quiz_id"]; ?>'" />
										</td>
										<td style="width: 75%; text-align: right; vertical-align: middle">
											...
											<select id="post_action" name="post_action">
												...
												<option value="new"<?php echo (($_SESSION[APPLICATION_IDENTIFIER]["tmp"]["post_action"] == "new") ? " selected=\"selected\"" : ""); ?>>Add a new question</option>
				...
				application_log("error", "Attempted to edit a quiz question in quiz [".$FORM_ID."] that has already been taken.");
			}
		} else {
			$ERROR++;
			...
			application_log("notice", "Failed to provide a valid quiz identifer [".$FORM_ID."] when attempting to edit a quiz question.");
		}
	} else {
		$ERROR++;
