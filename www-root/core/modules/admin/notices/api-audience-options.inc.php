...
//			$course_groups = $db->GetAll($query);
			...
			<tr>
				<td>&nbsp;</td>
				<td style="vertical-align: top"><label for="faculty_name" class="form-nrequired">Associated Recipients</label></td>
				<td>
					<table>
						<tbody>
								<tr>
										...
										<div id="event_audience_type_custom_options" style="<?php echo ($course_list && !$custom_audience ? "display: none; " : ""); ?>position: relative; margin-top: 10px;">
											...
											if (!$use_ajax) {
												...
												$query = "	SELECT a.*
															...
															AND b.`organisation_id` = '".$PROCESSED["org_id"]."'
											...
											if (!isset($PROCESSED["associated_cohorts"]) && !isset($PROCESSED["associated_students"]) && !isset($_POST["associated_cohorts"]) && !isset($_POST["associated_students"]) && isset($PROCESSED["org_id"])) {

												$query = "SELECT * FROM `notice_audience` WHERE `notice_id` = ".$db->qstr($NOTICE_ID);
												$audience = $db->GetAll($query);
												if($audience){
													foreach($audience as $amember){
														if(strpos($amember["audience_type"],"all:") !== false){
											...
											<select id="audience_type" name="target_audience" onchange="showMultiSelect();" style="width: 275px;">
												<option value="-1">-- Select an audience type --</option>
												<?php

													?>
													<option value="all:users"<?php echo ($target_audience=="all:users"?"selected=\"selected\"":"");?>>All users</option>
													<option value="all:students"<?php echo ($target_audience=="all:students"?"selected=\"selected\"":"");?>>All students</option>
													<option value="all:faculty"<?php echo ($target_audience=="all:faculty"?"selected=\"selected\"":"");?>>All faculty</option>
													<option value="all:staff"<?php echo ($target_audience=="all:staff"?"selected=\"selected\"":"");?>>All staff</option>
													<option value="cohorts">Cohorts of learners</option>
													<option value="course_list">Course lists</option>
											...
											<ul class="menu multiselect" id="audience_list" style="margin-top: 5px" <?php echo ($target_audience?"style=\"display:none;\"":"");?>>
											<?php
