...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_AAMC_CI"))) {
	...
	switch($STEP) {
		case 2 :
			...
			if ((isset($_POST["report_title"])) && ($report_title = clean_input($_POST["report_title"], array("notags", "trim")))) {
				if ($report_title != $REPORT["report_title"]) {
			...
			if (!$ERROR) {
				...
				if ($db->AutoExecute("reports_aamc_ci", $PROCESSED, "UPDATE", "raci_id=".$db->qstr($REPORT_ID))) {
					$url = ENTRADA_URL . "/admin/reports/aamc/manage?id=".$REPORT_ID;
					...
					application_log("success", "AAMC curriculum inventory report [".$REPORT_ID."] updated.");
				} else {
					add_error("There was a problem updating this report at this time. The system administrator was informed of this error; please try again later.");

					application_log("error", "There was an error updating an AAMC curriculum inventory report [".$REPORT_ID."]. Database said: ".$db->ErrorMsg());
				}
			}
	...
	switch($STEP) {
		...
		default :
			...
			?>
			<form action="<?php echo ENTRADA_RELATIVE; ?>/admin/reports/aamc/manage?section=edit&amp;id=<?php echo $REPORT_ID; ?>&amp;step=2" method="post" id="editAAMCCiReport">
				<table style="width: 100%" cellspacing="0" cellpadding="2" border="0" summary="Edit AAMC Curriclum Inventory Report">
					<colgroup>
						<col style="width: 3%" />
						<col style="width: 20%" />
						<col style="width: 77%" />
									...
									<tbody>
										<tr>
											<td style="width: 25%; text-align: left">
												<input type="button" class="button" value="Cancel" onclick="window.location='<?php echo ENTRADA_RELATIVE; ?>/admin/reports/aamc/manage?id=<?php echo $REPORT_ID; ?>'" />
											</td>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<textarea id="report_description" name="report_description" style="width: 100%; height: 150px" cols="70" rows="10"><?php echo html_encode($PROCESSED["report_description"]); ?></textarea>
							</td>
						</tr>
						<tr>
							<td colspan="3">&nbsp;</td>
