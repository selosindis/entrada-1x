...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_REPORTS"))) {
	...
	$BREADCRUMB[]	= array("url" => "", "title" => "Grant Eligible - Number of Students" );
	...
	if ($STEP == 2) {
		...
		$query		= "	SELECT c.`firstname`, c.`lastname`, c.`email`, a.`event_start`, a.`event_finish`
		...
		if ($results) {
			...
			<thead>
				<tr>
					...
					<td class="general">Event Start</td>
					<td class="general">Event Finish</td>
				</tr>
			</thead>
			<tbody>
			<?php
			...
			foreach ($results as $result) {
				if ((bool) $result["email"]) {
					...
					echo "	<td class=\"general\">".date("D M d/y", $result["event_start"])."</td>\n";
					echo "	<td class=\"general\">".date("D M d/y", $result["event_finish"])."</td>\n";
					echo "</tr>\n";
				}
			}
			?>
