...
<?php
	$BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/settings/organisations/manage/departments?section=delete&amp;org=".$ORGANISATION['organisation_id'], "title" => "Delete Departments");
	...
	if ($PROCESSED["remove_ids"]) {
		switch ($STEP) {
			case 2:
			 foreach ($PROCESSED["remove_ids"] as $id) {
				...
				$ONLOAD[] = "setTimeout('window.location=\\'".ENTRADA_URL."/admin/settings/organisations/manage/departments/?org=".$ORGANISATION_ID."\\'', 5000)";
				break;
			case 1:
			default:
			...
			<form action ="<?php echo ENTRADA_URL."/admin/settings/organisations/manage/departments/?section=delete&org=".$ORGANISATION_ID."&step=2";?>" method="post">
		...
		$ERROR++;
		$ERRORSTR[] = "No Departments were selected to be deleted. You will now be redirected to the Department index; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"".ENTRADA_URL."/admin/settings/organisations/manage/departments/?org=".$ORGANISATION_ID."\" style=\"font-weight: bold\">click here</a> to continue.";
