	...
	if ($PROCESSED["remove_ids"]) {
		switch($STEP){
			...
			if(isset($_POST["remove_ids"]) && is_array($_POST["remove_ids"]) && !empty($_POST["remove_ids"])){
				...
				$ERROR++;
				$ERRORSTR[] = "No Meta Data Types were selected to be deleted. You will now be redirected to the Meta Data index; this will happen <strong>automatically</strong> in 5 seconds or <a href=\"".ENTRADA_URL."/admin/settings/organisations/manage/metadata/?org=".$ORGANISATION_ID."\" style=\"font-weight: bold\">click here</a> to continue.";

				echo display_error();
			}
				$ONLOAD[] = "setTimeout('window.location=\\'".ENTRADA_URL."/admin/settings/organisations/manage/metadata/?org=".$ORGANISATION_ID."\\'', 5000)";
			break;
		case 1:
		default:			
			...
			<form action ="<?php echo ENTRADA_URL."/admin/settings/organisations/manage/hottopics/?section=delete&org=".$ORGANISATION_ID."&step=2";?>" method="post" id="delete_form">
