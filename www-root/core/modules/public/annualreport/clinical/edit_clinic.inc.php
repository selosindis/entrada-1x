...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($CLINICS_ID) {
		$query	= "SELECT * FROM `ar_clinics` WHERE `clinics_id`=".$db->qstr($CLINICS_ID);
					...
					<tr>
						...
						<?php
							for($i=$AR_PAST_YEARS; $i<=$AR_FUTURE_YEARS; $i++)
							{
								...
								else if(isset($clinicResult["year_reported"]) && $clinicResult["year_reported"] != '')
								{
									$defaultYear = $clinicResult["year_reported"];
								}
								else 
								{
