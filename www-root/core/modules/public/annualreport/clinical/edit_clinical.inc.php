...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($CLINICAL_ACTIVITY_ID) {
		$query	= "SELECT * FROM `ar_clinical_activity` WHERE `clinical_activity_id`=".$db->qstr($CLINICAL_ACTIVITY_ID);
					...
					<tr>
						...
						<?php
							for($i=$AR_PAST_YEARS; $i<=$AR_FUTURE_YEARS; $i++)
							{
								...
								else if(isset($clinicalResult["year_reported"]) && $clinicalResult["year_reported"] != '')
								{
									$defaultYear = $clinicalResult["year_reported"];
								}
								else 
								{
