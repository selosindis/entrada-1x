...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($CONSULTS_ID) {
		$query	= "SELECT * FROM `ar_consults` WHERE `consults_id`=".$db->qstr($CONSULTS_ID);
					...
					<tr>
						...
						<?php
							for($i=$AR_PAST_YEARS; $i<=$AR_FUTURE_YEARS; $i++)
							{
								...
								else if(isset($consultResult["year_reported"]) && $consultResult["year_reported"] != '')
								{
									$defaultYear = $consultResult["year_reported"];
								}
								else 
								{
