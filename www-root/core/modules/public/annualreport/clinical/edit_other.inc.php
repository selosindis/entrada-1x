...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($OTHER_ACTIVITY_ID) {
		$query	= "SELECT * FROM `ar_other_activity` WHERE `other_activity_id`=".$db->qstr($OTHER_ACTIVITY_ID);
					...
					<tr>
						...
						<?php
							for($i=$AR_PAST_YEARS; $i<=$AR_FUTURE_YEARS; $i++)
							{
								...
								else if(isset($otherResult["year_reported"]) && $otherResult["year_reported"] != '')
								{
									$defaultYear = $otherResult["year_reported"];
								}
								else 
								{
