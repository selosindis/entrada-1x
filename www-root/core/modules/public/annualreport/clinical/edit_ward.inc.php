...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($WARD_SUPERVISION_ID) {
		$query	= "SELECT * FROM `ar_ward_supervision` WHERE `ward_supervision_id`=".$db->qstr($WARD_SUPERVISION_ID);
					...
					<tr>
						...
						<?php
							for($i=$AR_PAST_YEARS; $i<=$AR_FUTURE_YEARS; $i++)
							{
								...
								else if(isset($wardResult["year_reported"]) && $wardResult["year_reported"] != '')
								{
									$defaultYear = $wardResult["year_reported"];
								}
								else 
								{
