...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($CLINICAL_EDUCATION_ID) {
		$query	= "SELECT * FROM `ar_clinical_education` WHERE `clinical_education_id`=".$db->qstr($CLINICAL_EDUCATION_ID);
					...
					<tr>
						...
						<?php
							for($i=$AR_PAST_YEARS; $i<=$AR_FUTURE_YEARS; $i++)
							{
								...
								else if(isset($clinicalEducationResult["year_reported"]) && $clinicalEducationResult["year_reported"] != '')
								{
									$defaultYear = $clinicalEducationResult["year_reported"];
								}
								else 
								{
