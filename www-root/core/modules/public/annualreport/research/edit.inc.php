...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($RESEARCH_ID) {
		$query	= "SELECT * FROM `ar_research` WHERE `research_id`=".$db->qstr($RESEARCH_ID);
					...
					<tr>
						...
						<?php
							for($i=$AR_PAST_YEARS; $i<=$AR_FUTURE_YEARS; $i++)
							{
								...
								else if(isset($researchResult["year_reported"]) && $researchResult["year_reported"] != '')
								{
									$defaultYear = $researchResult["year_reported"];
								}
								else 
								{
