...
if((!defined("PARENT_INCLUDED")) || (!defined("IN_ANNUAL_REPORT"))) {
	...
	if($RESEARCH_ID) {
		$query	= "SELECT * FROM `ar_scholarly_activity` WHERE `scholarly_activity_id`=".$db->qstr($RESEARCH_ID);
					...
					<tr>
						...
						<?php
							for($i=$AR_PAST_YEARS; $i<=$AR_FUTURE_YEARS; $i++)
							{
								...
								else if(isset($scholarlyResult["year_reported"]) && $scholarlyResult["year_reported"] != '')
								{
									$defaultYear = $scholarlyResult["year_reported"];
								}
								else 
								{
