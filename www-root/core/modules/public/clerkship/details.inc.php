 ...
 *
 * @version $Id: details.inc.php 1171 2010-05-01 14:39:27Z ad29 $
*/
	...
	switch ($_SESSION["permissions"][$_SESSION[APPLICATION_IDENTIFIER]["tmp"]["proxy_id"]]["group"]) {
		case "student" :
			...
			$query	= "	SELECT *
						...
						LEFT JOIN `global_lu_provinces` AS f
						ON c.`province_id` = f.`province_id`
						LEFT JOIN `global_lu_countries` AS g
						ON c.`countries_id` = g.`countries_id`
						...
						AND d.`econtact_id` = ".$db->qstr($_SESSION[APPLICATION_IDENTIFIER]["tmp"]["proxy_id"])."
						AND d.`aschedule_status` = 'published'
						AND e.`apartment_status` = 'published'";
			$result	= $db->GetRow($query);
			if ($result) {
				$BREADCRUMB[]	= array("url" => "", "title" => "Rotation Details");

				$max_occupants	= (int) $result["max_occupants"];
				$apt_occupants	= clerkship_apartment_occupants($result["apartment_id"], $result["event_start"], $result["event_finish"]);
						...
						if ($num_occupants) {
							...
							if (is_array($apt_occupants)) {
								foreach ($apt_occupants as $occupant) {
									...
									echo "			<td class=\"title\"><a href=\"".ENTRADA_URL."/people?profile=".html_encode($occupant["username"])."\" target=\"_blank\">".html_encode($occupant["firstname"]." ".$occupant["lastname"])."</a></td>\n";
