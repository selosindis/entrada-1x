...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_CLERKSHIP"))) {
	...
	if ($RECORD_ID) {
		...
		if ($PROCESSED) {
			$rotation_end = false;
			if ($PROCESSED["updated_date"] < strtotime("-1 week")) {
				$query = "	SELECT a.`event_finish` FROM `".CLERKSHIP_DATABASE."`.`events` AS a
							INNER JOIN `".CLERKSHIP_DATABASE."`.`event_contacts` AS b
							ON a.`event_id` = b.`event_id`
							WHERE a.`rotation_id` = 
							(
								SELECT `rotation_id` FROM `".CLERKSHIP_DATABASE."`.`events`
								WHERE `event_id` = ".$db->qstr($PROCESSED["rotation_id"])."
								AND `rotation_id` > 0
							)
							AND b.`etype_id` = ".$db->qstr($_SESSION["details"]["id"])."
							AND a.`event_type` = 'clinical'
							ORDER BY a.`event_finish` DESC";
				$rotation_end = $db->GetOne($query);
			}
			if (($PROCESSED["updated_date"] > strtotime("-1 week")) || ($rotation_end > strtotime("-1 week"))) {
				...
				switch ($STEP) {
					...
					$encounter_date = validate_single_calendar("encounter", true);	
					...
					if (!$ERROR && (!isset($_POST["allow_save"]) || $_POST["allow_save"])) {
						...
						if ($db->AutoExecute("`".CLERKSHIP_DATABASE."`.`logbook_entries`", $PROCESSED, "UPDATE", "`lentry_id` = ".$db->qstr($RECORD_ID))) {
							...
							application_log("success", "Patient encounter [".$ENTRY_ID."] updated in the system.");
						} else {
							$ERROR++;
					...
					default :
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<?php 
									$query	= "SELECT a.* FROM `".CLERKSHIP_DATABASE."`.`events` AS a LEFT JOIN `".CLERKSHIP_DATABASE."`.`event_contacts` AS b ON a.`event_id` = b.`event_id` WHERE b.`etype_id` = ".$db->qstr($_SESSION["details"]["id"])." AND a.`event_id` = ".$db->qstr(((int)$PROCESSED["rotation_id"]));
									...
									<?php
									$query		= "SELECT a.* FROM `".CLERKSHIP_DATABASE."`.`events` AS a LEFT JOIN `".CLERKSHIP_DATABASE."`.`event_contacts` AS b ON a.`event_id` = b.`event_id` WHERE `etype_id` = ".$db->qstr($_SESSION["details"]["id"]);
							...
							*/ ?>
							<tr>
								...
								<td>
									...
									$query = "SELECT * FROM `".CLERKSHIP_DATABASE."`.`global_lu_rotations` WHERE `rotation_id` = (SELECT `rotation_id` FROM `".CLERKSHIP_DATABASE."`.`events` WHERE `event_id` = ".$db->qstr($PROCESSED["event_id"]).")";
									...
									if ($rotation) {
										...
										?>
											<input type="radio" name="objective_display_type" id="objective_display_type_defficient" onclick="showDefficientObjectives()" /> <label for="objective_display_type_defficient">Show only clinical presentations which are defficient from past rotations.</label>
										<?php
										}
										?>
										<br /><br />
									...
									if ($results) {
										foreach ($results as $result) {
											...
											if ($children) {
												foreach ($children as $child) {
													echo "<option id=\"defficient-obj-item-".$child["objective_id"]."\" value=\"".(int) $child["objective_id"]."\">".html_encode($child["objective_name"])."</option>\n";
												}
											}
										}
									}
								...
								<td style="vertical-align: top;">
									...
									<td style="vertical-align: top;">
										...
										<?php
											...
											$defficient_procedures = $db->GetAll($query);
											if ($rotation) {
												$query = "	SELECT DISTINCT a.* FROM `".CLERKSHIP_DATABASE."`.`logbook_lu_procedures` AS a
															LEFT JOIN `".CLERKSHIP_DATABASE."`.`logbook_preferred_procedures` AS b
															ON b.`lprocedure_id` = a.`lprocedure_id`
												...
												if ($preferred_procedures) {
													...
													<?php
													if ($defficient_procedures) {
													?>
														<input type="radio" name="procedure_display_type" id="procedure_display_type_defficient" onclick="showDefficientProcedures()" /> <label for="procedure_display_type_defficient">Show only clinical tasks which are defficient from past rotations.</label>
													<?php
													}
													?>
													<br /><br />
													<?php
												} elseif ($defficient_procedures) {
												?>
													<input type="radio" name="procedure_display_type" id="procedure_display_type_all" onclick="showAllProcedures()" /> <label for="procedure_display_type_all">Show all clinical tasks</label><br />
													<input type="radio" name="procedure_display_type" id="procedure_display_type_defficient" onclick="showDefficientProcedures()" /> <label for="procedure_display_type_defficient">Show only clinical tasks which are defficient from past rotations.</label>
												<?php
										...
										echo "</select>\n";
										echo "<select id=\"defficient_procedure_id\" name=\"defficient_procedure_id\" onchange=\"addProcedure(this.value, 0)\" style=\"width: 95%; display: none;\">\n";
										echo "<option value=\"0\"".((!isset($PROCESSED["objective_id"])) ? " selected=\"selected\"" : "").">-- Select Clinical Tasks --</option>\n";
										if ($defficient_procedures) {
											foreach ($defficient_procedures as $result) {
												echo "<option id=\"defficient-proc-item-".$result["lprocedure_id"]."\" value=\"".(int) $result["lprocedure_id"]."\">".html_encode($result["procedure"])."</option>\n";
											}
										}
										echo "</select>\n";
												...
												<tr id="procedure_<?php echo $procedure_id["lprocedure_id"]; ?>_row">
													...
													<td style="text-align: right">
														...
														<select name="proc_participation_level[<?php echo $procedure_id["lprocedure_id"]; ?>]" id="proc_<?php echo $procedure_id["lprocedure_id"]; ?>_participation_level" style="width: 150px">
															...
															<?php /*
															<option value="1" <?php echo ($procedure_id["level"] == 1 ? "selected=\"selected\"" : ""); ?>>Performed with help</option>
															<option value="2" <?php echo ($procedure_id["level"] == 2 ? "selected=\"selected\"" : ""); ?>>Performed independently</option>
															*/ ?>
														</select>
													</td>
												</tr>
				...
				$ERROR++;
				$ERRORSTR[]	= "The selected entry was logged too long ago to be edited.<br /><br />If you believe you are receiving this message in error please contact <a href=\"mailto:".html_encode($AGENT_CONTACTS["administrator"]["email"])."\">".html_encode($AGENT_CONTACTS["administrator"]["name"])."</a> for assistance.";
			
				echo display_error();
			
				application_log("error", "Error, Entry ID [".$RECORD_ID."] was logged to long ago for editing a logbook entry in module [".$MODULE."].");
			}
		} else {
			$ONLOAD[]	= "setTimeout('window.location=\\'".ENTRADA_URL."/".$MODULE."\\'', 15000)";
			...
			application_log("error", "Error, invalid Entry ID [".$RECORD_ID."] supplied for editing a logbook entry in module [".$MODULE."].");
		}
	} else {
		$ONLOAD[]	= "setTimeout('window.location=\\'".ENTRADA_URL."/".$MODULE."\\'', 15000)";
