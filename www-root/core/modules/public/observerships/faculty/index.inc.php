...
if (!defined("IN_OBSERVERSHIPS_FACULTY")) {
	exit;
} elseif ((!isset($_SESSION["isAuthorized"])) || (!$_SESSION["isAuthorized"])) {
	header("Location: ".ENTRADA_URL);
	exit;
} elseif(!$ENTRADA_ACL->isLoggedInAllowed('observerships', 'read',true) || $_SESSION["details"]["group"] != "faculty") {
...
?>
Table showing incoming requests with option to accept or reject requests (reject can be accompanied by note for reason)

link to review completed requests

Table showing completed requests requiring feedback
