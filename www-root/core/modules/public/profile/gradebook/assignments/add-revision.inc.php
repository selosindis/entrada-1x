...
if ($RECORD_ID) {
	$query			= "SELECT * FROM `assignment_files` WHERE `assignment_id` = ".$db->qstr($RECORD_ID)." AND `proxy_id` = ".$db->qstr($ENTRADA_USER->getProxyId())." AND `file_active` = '1'";
		...
		if ($permitted) {
				...
				case 2 :
					if (isset($_FILES["uploaded_file"])) {
						...
						$query = "	SELECT a.`file_version`,b.`afile_id` FROM `assignment_file_versions` AS a 
										...
										AND b.`proxy_id` = ".$db->qstr($ENTRADA_USER->getProxyId())."
			...
			switch($STEP) {
				...
				default :					
					...
					$query = "	SELECT *  FROM `assignment_files` WHERE `assignment_id` = ".$db->qstr($RECORD_ID)." AND `proxy_id` = ".$db->qstr($ENTRADA_USER->getProxyId());
					$original_file = $db->GetRow($query);					
					?>


					<form id="upload-file-form" action="<?php echo ENTRADA_URL."/profile/gradebook/assignments?section=add-revision&id=".$RECORD_ID."&step=2"; ?>" method="post" enctype="multipart/form-data">
