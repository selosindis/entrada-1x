jQuery(function($) {
            ...
            if (jsonResponse.data != "bottom_level") {
                ...
                $(jsonResponse.data.objectives).each(function(i, v) {
                    ...
                    if (v.has_children == true) {
                        var matrix_header_row_objective_cell_link = $(document.createElement("a")).attr({"href": "#objective-id=" + v.objective_id, "data-objective-id": v.objective_id});
                        ...
                        var matrix_header_objective_cell_span = $(document.createElement("span")).html(v.objective_name);
                ...
                var matrix_body = $(document.createElement("tbody"))
                var last_term = "";
                var term_id = 0;
                for (var i = 0; i < jsonResponse.data.courses.length; i++) {
                    ...
                    var l = 1;
                    for(var k in jsonResponse.data.courses[i].objectives) {
                        if (jsonResponse.data.courses[i].objectives[k] != false) {
                            if ($("#curriculum-matrix-appendix ul .objective-" + k).children("ul").length <= 0) {
                                var child_list = $(document.createElement("ul"));
                                $("#curriculum-matrix-appendix ul .objective-" + k).append(child_list);
                            }
                            if (jsonResponse.data.courses[i].objectives[k] != false) {
                                var course_list_item = $(document.createElement("li")).html(jsonResponse.data.courses[i].course_code + " - " + jsonResponse.data.courses[i].course_name + " - " + levels[jsonResponse.data.courses[i].objectives[k]] + " Level Objective");
                                $("#curriculum-matrix-appendix ul .objective-" + k + " ul").append(course_list_item);
                            }
                        }
                        var matrix_body_course_row_cell_obj = $(document.createElement("td")).html(jsonResponse.data.courses[i].objectives[k] != false ? "<a class=\"objective-details\" href=\"#objective-modal\" data-course-id=\""+jsonResponse.data.courses[i].course_id+"\" data-objective-id=\""+k+"\" data-toggle=\"modal\">" + jsonResponse.data.courses[i].objectives[k] + "</a>" : "");
                ...
                breadcrumb_list_entry_link.append(clicked_heading.html()).attr("data-objective-id", clicked_heading.data("objective-id"));
    ...
    $("#curriculum-matrix-parent").on("submit", function(e) {
        ...
        var breadcrumb_list_entry_link = $(document.createElement("a"));
        breadcrumb_list_entry_link.append($("#objective-set option[value=" + $("#objective-set").val() + "]").html()).attr("data-objective-id", $("#objective-set").val());
