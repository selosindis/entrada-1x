 ...
 *
 * @version $Id: quiz-wizard-event.php 1171 2010-05-01 14:39:27Z ad29 $
*/
...
if((!isset($_SESSION["isAuthorized"])) || (!$_SESSION["isAuthorized"])) {
	...
	if ((isset($_GET["qid"])) && ((int) trim($_GET["qid"]))) {
		$EQUIZ_ID = (int) trim($_GET["qid"]);
	}

	$PAGE_META["title"] = "Quiz Wizard";
	...
	if ($EVENT_ID) {
		...
		if($event_record) {
			if(!$ENTRADA_ACL->amIAllowed(new EventContentResource($EVENT_ID, $event_record['course_id'], $event_record['organisation_id']), 'update')) {
				...
				$query		= "SELECT `quiz_id` FROM `event_quizzes` WHERE `event_id` = ".$db->qstr($EVENT_ID);
				...
				switch($ACTION) {
					case "edit" :
						...
						if ($EQUIZ_ID) {
							$query			= "SELECT * FROM `event_quizzes` WHERE `equiz_id` = ".$db->qstr($EQUIZ_ID);
							...
							if ($equiz_record) {
								...
								switch($STEP) {
									case 4 :
										...
										if ($db->AutoExecute("event_quizzes", $PROCESSED, "UPDATE", "equiz_id = ".$db->qstr($EQUIZ_ID))) {
											$ONLOAD[] = "parentReload()";
											?>
											<div id="wizard">
												<div id="header">
												...
												<div id="footer">
													...
													<button onclick="window.location='<?php echo ENTRADA_URL; ?>/quiz-wizard-event.php?id=<?php echo $EVENT_ID; ?>&amp;qid=<?php echo $EQUIZ_ID; ?>&amp;action=edit'" style="float: right; margin: 4px 10px 4px 0px">Edit Again</button>
												</div>
											</div>
											<?php
										} else {
											...
											application_log("error", "Unable to update an attached quiz [".$PROCESSED["quiz_id"]."] inder event [".$EVENT_ID."]. Database said: ".$db->ErrorMsg());
											?>
											<form action="<?php echo ENTRADA_URL; ?>/quiz-wizard-event.php?action=edit&amp;id=<?php echo $EVENT_ID; ?>&amp;qid=<?php echo $EQUIZ_ID; ?>" method="post">
											...
											<div id="wizard">
												...
												<div id="footer">
													<button id="close-button" onclick="closeWizard()">Close</button>
													<input type="submit" id="back-button" name="back_button" value="Previous Step" />
												</div>
											</div>
											</form>
											<?php
									...
									case 3 :
										...
										<div id="wizard">
											...
											<div id="footer">
												<button id="close-button" onclick="closeWizard()">Close</button>
												<input type="submit" id="next-button" name="next_button" value="Finish" />
												<input type="submit" id="back-button" name="back_button" value="Previous Step" />
											</div>
										</div>
										</form>
									...
									case 2 :
										...
										<div id="wizard">
											...
											<div id="footer">
												<button id="close-button" onclick="closeWizard()">Close</button>
												<input type="submit" id="next-button" name="next_button" value="Next Step" />
												<input type="submit" id="back-button" name="back_button" value="Previous Step" />
											</div>
										</div>
										</form>
								...
								// Error, unable to find the equiz_id record.
							}
						} else {
							// Error, there was no equiz_id provided.
						}
					break;
					case "add" :
					default :
							...
							case 5 :
								...
								if ($db->AutoExecute("event_quizzes", $PROCESSED, "INSERT")) {
									...
									<div id="wizard">
										...
										<div id="footer">
											<button id="close-button" onclick="closeWizard()">Close</button>
											<button onclick="window.location='<?php echo ENTRADA_URL; ?>/quiz-wizard-event.php?id=<?php echo $EVENT_ID; ?>&amp;action=add'" style="float: right; margin: 4px 10px 4px 0px">Add Another</button>
										</div>
									</div>
									<?php
								} else {
									...
									application_log("error", "Unable to attach quiz [".$PROCESSED["quiz_id"]."] to event [".$EVENT_ID."]. Database said: ".$db->ErrorMsg());
									?>
									<form action="<?php echo ENTRADA_URL; ?>/quiz-wizard-event.php?action=add&amp;id=<?php echo $EVENT_ID; ?>" method="post">
									<input type="hidden" name="step" value="<?php echo $STEP; ?>" />
									<?php
									foreach ($PROCESSED as $key => $value) {
										echo "<input type=\"hidden\" name=\"".html_encode($key)."\" value=\"".html_encode($value)."\" />";
