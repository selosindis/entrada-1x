											...
											if ($db->AutoExecute("attached_quizzes", $PROCESSED, "UPDATE", "aquiz_id = ".$db->qstr($AQUIZ_ID))) {
												...
												<div id="wizard">
													...
													<div id="footer">
														...
														<button onclick="window.location='<?php echo ENTRADA_URL; ?>/quiz-wizard.php?id=<?php echo $RECORD_ID; ?>&amp;qid=<?php echo $AQUIZ_ID; ?>&amp;action=edit'" style="float: right; margin: 4px 10px 4px 0px">Edit Again</button>
													</div>
												</div>
												<?php
											} else {
												...
												?>
												<form action="<?php echo ENTRADA_URL; ?>/quiz-wizard.php?type=<?php echo $QUIZ_TYPE; ?>&amp;action=edit&amp;id=<?php echo $RECORD_ID; ?>&amp;qid=<?php echo $AQUIZ_ID; ?>" method="post">
								...
								case 5 :
									...
									if ($db->AutoExecute("attached_quizzes", $PROCESSED, "INSERT")) {
										...
										<div id="wizard">
											...
											<div id="footer">
												<button id="close-button" onclick="closeWizard()">Close</button>
												<button onclick="window.location='<?php echo ENTRADA_URL; ?>/quiz-wizard.php?id=<?php echo $RECORD_ID; ?>&amp;action=add'" style="float: right; margin: 4px 10px 4px 0px">Add Another</button>
											</div>
										</div>
										<?php
									} else {
										...
										?>
										<form action="<?php echo ENTRADA_URL; ?>/quiz-wizard.php?type=<?php echo $QUIZ_TYPE; ?>&amp;action=add&amp;id=<?php echo $RECORD_ID; ?>" method="post">
										<input type="hidden" name="step" value="<?php echo $STEP; ?>" />
										<?php
										foreach ($PROCESSED as $key => $value) {
											echo "<input type=\"hidden\" name=\"".html_encode($key)."\" value=\"".html_encode($value)."\" />";
